...
async function getChatContext(chatId: string): Promise<{ chat: Chat; context: ChatContextMessage[] } | null> {
  const { data, error } = await sb.from('chats').select('*').eq('id', chatId).single();
  if (error) {
    log.error(`fetch chat ${chatId} failed`, error.message);
    return null;
  }
  return { chat: data, context: data.context ?? [] };
}

async function completeTask(task: Task, result: string, newContext: ChatContextMessage[]) {
  const [tUpd, cUpd] = await Promise.all([
    sb.from('tasks').update({ status: 'completed', result, error: null }).eq('id', task.id),
    sb.from('chats').update({ context: newContext }).eq('id', task.chat_id),
  ]);
...
async function processTask(taskId: string) {
  ...
  try {
    ...
    const ctx = await getChatContext(task.chat_id);
    if (!ctx) throw new Error('Chat context not available');

    const result = await executeTaskWithClaude(task, ctx.context, claudeConfig);

    const now = new Date().toISOString();
    const newContext: ChatContextMessage[] = [
      ...ctx.context,
      { role: 'user', content: `[Task: ${task.title}]\n${task.instructions}`, task_id: task.id, at: now },
      { role: 'assistant', content: result, task_id: task.id, at: now },
    ];
    await completeTask(task, result, newContext);
